...
==============================================
Dataset  : pelmers/github-repository-metadata-with-5-stars
           https://www.kaggle.com/datasets/pelmers/github-repository-metadata-with-5-stars

File layout expected on disk:
  DBMS_database\
    repos_folder\
      repos.json          <-- your ~600 MB Kaggle download (rename to repos.json)
    DB\
      ingest.py           <-- THIS FILE
      Py_file.py
      schema.sql
...
Phases:
  Phase 2 — Kaggle ingestion  (repos.json -> repositories + repo_topics + repository_topics)
  Phase 3 — GitHub API fetch by USERNAME (GET /users/{username}/repos)
  Phase 4 — Kaggle <-> GitHub linking  (exact -> name -> fuzzy)
  Phase 5 — Issue ingestion per linked repo

Schema targeted (matches schema.sql exactly):
  repositories(repo_id, github_id, owner, name, full_name,
               description, stars, forks, language, html_url, image_path, created_at)
  repo_topics(topic_id, name)
  repository_topics(repo_id, topic_id)
  issues(issue_id, github_issue_id, repo_id, title, body, state,
         complexity_score, estimated_time_hours, github_url,
         screenshot_path, created_at, updated_at)
  issue_labels(label_id, issue_id, name, color)   -- UNIQUE(issue_id, name)

Install:
  pip install psycopg2-binary pandas requests python-dotenv tqdm
"""
