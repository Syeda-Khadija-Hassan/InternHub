"""
Py_file.py — Alternative pipeline for the GitHub Recommender project.
=====================================================================
File layout expected on disk:
  DBMS_database\
    repos_folder\
      repos.json          <-- your ~600 MB Kaggle download
    DB\
      Py_file.py          <-- THIS FILE
      ingest.py
      schema.sql
      queries.sql

This script targets the SAME schema.sql as ingest.py.
It is a self-contained alternative — you can use either file.

Differences from ingest.py:
  - Uses psycopg2.extras.execute_values for bulk inserts (faster).
  - Provides run_api_sync(repo_list) for syncing a specific list of repos.
  - Simpler single-file structure, no class hierarchy.
